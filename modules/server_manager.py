...
def assign_server_values_list(self):
    ...
    self.c.execute("select ipaddress, queryport, rconport, rconpw FROM server WHERE alias = (:select_alias)", {
                    'select_alias': selection})
    extract = self.c.fetchone()
    self.conn.commit()
    self.gui.entry_ip.setText(extract[0])
    self.gui.entry_queryport.setText(str(extract[1]))
    self.gui.entry_rconport.setText(str(extract[2]))
    self.gui.entry_rconpw.setText(extract[3])
    query.checkandgoquery(self)
...
def server_delete(self):
    ...
    self.c.execute(
            "select alias, ipaddress, queryport, rconport, rconpw from server where id=:fid", {'fid': int(server_delete_id)})
    new_selection = self.c.fetchone()
    if new_selection is not None:
        self.gui.server_alias.setText(new_selection[0])
        self.gui.server_alias.setCursorPosition(1)
        self.gui.server_ip.setText(new_selection[1])
        self.gui.server_query.setText(str(new_selection[2]))
        self.gui.server_rconport.setText(str(new_selection[3]))
        self.gui.server_rconpw.setText(new_selection[4])
        self.gui.btn_server_delete.setEnabled(True)
        self.gui.btn_server_modify.setEnabled(True)
    else:
        self.gui.btn_server_delete.setEnabled(False)
...
def create_db_backup(self):
    ...
    target_dir = QtWidgets.QFileDialog.getExistingDirectory(
            self, 'Select target folder for DB backup', db_backup_directory)

    db_backup_filename = (target_dir + '\\' +
